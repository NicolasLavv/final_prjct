...
def post_new_order(data): # создаём новый заказ, который содержит токен Authorization
    return requests.post(configuration.URL_SERVICE + configuration.CREATE_USER_ORDER,  # url + api запрос на создание заказа
...
def get_an_order_track(): #функция на получение трека
